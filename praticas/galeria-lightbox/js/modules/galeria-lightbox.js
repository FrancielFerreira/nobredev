export default function initGaleria() {
  ...
  Array.from(galeria).forEach((img, index) =>
    img.addEventListener("click", () => activeGaleria(index))
  );

  function closeGaleriaContainer(e) {
    e.target === this || e.target === btnCloseGaleria
      ? galeriaContainer.classList.remove("ativo")
      : 0;
  }
